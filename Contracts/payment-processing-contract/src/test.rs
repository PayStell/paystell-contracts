...
use crate::{
    ...
    storage::Storage,
    types::{
        BatchMerchantRegistration, BatchPayment, BatchTokenAddition, GasEstimate, MerchantCategory,
        NonceTracker, PaymentOrder, PaymentQueryFilter, PaymentQueryResult, PaymentRecordStatus,
        PaymentStats, PaymentStatus, ProfileUpdateData, RefundRequest, RefundStatus, SortField,
        SortOrder,
    },
...
fn register_test_merchant(client: &PaymentProcessingContractClient, env: &Env, merchant: &Address) {
...
fn test_gas_estimation() {
    ...
    let batch_estimate =
        client.estimate_gas_for_batch_operation(&Symbol::new(&env, "proc_pay"), &3);
...
fn test_initiate_multisig_payment_success() {
    ...
    let payment_id = client
        .initiate_multisig_payment(&amount, &token, &recipient, &signers, &threshold, &expiry);
...
fn test_initiate_multisig_payment_invalid_threshold() {
    ...
    let result = client
        .try_initiate_multisig_payment(&amount, &token, &recipient, &signers, &threshold, &expiry);
...
fn test_pause_until_duration_not_passed() {
    ...
    env.ledger().set_timestamp(80); // Still before pause_until (110)

    // Should still be paused - verify
    let is_paused = client.is_paused();
    assert_eq!(is_paused, true);

    // Trying to register a new merchant should panic with ContractPaused
    let merchant2 = Address::generate(&env);
    register_test_merchant(&client, &env, &merchant2);
}
...
// Payment History Query & Management Tests
...
fn test_get_merchant_payment_history() {
    ...
    client.add_supported_token(&merchant, &token);
    client.set_admin(&admin);
    client.set_fee(&0, &admin, &token);

    // Create multiple payments
    token_admin.mint(&payer, &10000);
    let signature = BytesN::from_array(&env, &[1u8; 64]);
    let merchant_public = BytesN::from_array(&env, &[2u8; 32]);
    ...
    for i in 1..=5 {
        let order = PaymentOrder {
            ...
            nonce: i,
            expiration: (env.ledger().timestamp() + 1000) as u32,
            order_id: String::from_str(&env, &format!("ORDER_{}", i)),
            fee_amount: 0,
        };
        env.mock_all_auths();
        client.process_payment_with_signature(&payer, &order, &signature, &merchant_public);
    ...
    env.mock_all_auths();
    let result = client.get_merchant_payment_history(
        &merchant,
        &None,
        &10,
        &None,
...
#[test]
fn test_get_payer_payment_history() {
    let env = Env::default();
    let contract_id = env.register(PaymentProcessingContract {}, ());
    let client = PaymentProcessingContractClient::new(&env, &contract_id);
    ...
    // Create payments
    token_admin.mint(&payer, &5000);
    let signature = BytesN::from_array(&env, &[3u8; 64]);
    let merchant_public = BytesN::from_array(&env, &[4u8; 32]);
    ...
    for i in 1..=3 {
        let order = PaymentOrder {
            ...
            nonce: i,
            expiration: (env.ledger().timestamp() + 1000) as u32,
            order_id: String::from_str(&env, &format!("PAYER_ORDER_{}", i)),
            fee_amount: 0,
        };
        env.mock_all_auths();
        client.process_payment_with_signature(&payer, &order, &signature, &merchant_public);
    ...
    env.mock_all_auths();
    let result = client.get_payer_payment_history(
        &payer,
        &None,
        &10,
        &None,
        &Some(SortField::Amount),
...
fn test_get_payment_by_id() {
    ...
    // Query by ID as payer
    env.mock_all_auths();
    let payment = client.get_payment_by_id(&payer, &order.order_id);
    assert_eq!(payment.order_id, order.order_id);
    assert_eq!(payment.amount, 1000);

    // Query by ID as merchant
    env.mock_all_auths();
    let payment2 = client.get_payment_by_id(&merchant, &order.order_id);
    assert_eq!(payment2.order_id, order.order_id);
}
...
fn test_get_global_payment_stats() {
    ...
    for i in 1..=5 {
        let order = PaymentOrder {
            ...
            nonce: i,
            expiration: (env.ledger().timestamp() + 1000) as u32,
            order_id: String::from_str(&env, &format!("STATS_ORDER_{}", i)),
            fee_amount: 0,
        };
        env.mock_all_auths();
        client.process_payment_with_signature(&payer, &order, &signature, &merchant_public);
    ...
    assert_eq!(stats.total_amount, 1500); // 100 + 200 + 300 + 400 + 500
...
fn test_update_payment_status() {
    ...
    let order = PaymentOrder {
        ...
        nonce: 1,
        expiration: (env.ledger().timestamp() + 1000) as u32,
        order_id: String::from_str(&env, "UPDATE_ORDER_1"),
        fee_amount: 0,
    };
    let signature = BytesN::from_array(&env, &[9u8; 64]);
    let merchant_public = BytesN::from_array(&env, &[10u8; 32]);

    env.mock_all_auths();
    client.process_payment_with_signature(&payer, &order, &signature, &merchant_public);

    // Update payment status (refunded amount)
    env.mock_all_auths();
    client.update_payment_status(&merchant, &order.order_id, &500);

    // Verify update
    env.mock_all_auths();
    let payment = client.get_payment_by_id(&merchant, &order.order_id);
    assert_eq!(payment.refunded_amount, 500);
}

#[test]
fn test_archive_payment_record() {
    let env = Env::default();
    let contract_id = env.register(PaymentProcessingContract {}, ());
    let client = PaymentProcessingContractClient::new(&env, &contract_id);
    ...
    let order = PaymentOrder {
        ...
        nonce: 1,
        expiration: (env.ledger().timestamp() + 1000) as u32,
        order_id: String::from_str(&env, "ARCHIVE_ORDER_1"),
        fee_amount: 0,
    };
    let signature = BytesN::from_array(&env, &[11u8; 64]);
    let merchant_public = BytesN::from_array(&env, &[12u8; 32]);
    ...
    // Verify payment is archived (should not be found in active payments)
    // Note: Archived payments are removed from active storage, so querying will fail
    // In a real scenario, you'd query the archive storage separately
}

#[test]
fn test_set_payment_cleanup_period() {
    let env = Env::default();
    let contract_id = env.register(PaymentProcessingContract {}, ());
    let client = PaymentProcessingContractClient::new(&env, &contract_id);
    ...
    // Set cleanup period (90 days)
    let period = 90 * 24 * 60 * 60;
    env.mock_all_auths();
    client.set_payment_cleanup_period(&admin, &period);

    // Verify it was set (by attempting cleanup - should return 0 if no expired payments)
    env.mock_all_auths();
    let cleaned = client.cleanup_expired_payments(&admin);
    assert_eq!(cleaned, 0);
}

#[test]
fn test_payment_history_with_filters() {
    let env = Env::default();
    let contract_id = env.register(PaymentProcessingContract {}, ());
    let client = PaymentProcessingContractClient::new(&env, &contract_id);
    ...
    token_admin.mint(&payer, &10000);
    let signature = BytesN::from_array(&env, &[13u8; 64]);
    let merchant_public = BytesN::from_array(&env, &[14u8; 32]);
    ...
    for i in 1..=5 {
        let order = PaymentOrder {
            ...
            nonce: i,
            expiration: (env.ledger().timestamp() + 1000) as u32,
            order_id: String::from_str(&env, &format!("FILTER_ORDER_{}", i)),
            fee_amount: 0,
        };
        env.mock_all_auths();
        client.process_payment_with_signature(&payer, &order, &signature, &merchant_public);
    ...
    // Query with amount filter (200-400)
    ...
    env.mock_all_auths();
    let result =
        client.get_merchant_payment_history(&merchant, &None, &10, &Some(filter), &None, &None);

    // Should return payments with amounts 200, 300, 400
    assert_eq!(result.records.len(), 3);
}

#[test]
fn test_unauthorized_payment_query() {
    let env = Env::default();
    let contract_id = env.register(PaymentProcessingContract {}, ());
    let client = PaymentProcessingContractClient::new(&env, &contract_id);
    ...
    // Register the merchant first so it exists
    env.mock_all_auths();
    register_test_merchant(&client, &env, &merchant);

    // With mock_all_auths, the query should succeed for a registered merchant
    // This tests that authorized queries work correctly
    env.mock_all_auths();
    let result = client.get_merchant_payment_history(&merchant, &None, &10, &None, &None, &None);

    // Should return empty records for a merchant with no payments
...
fn test_invalid_query_limit() {
    ...
    client.get_merchant_payment_history(
        &merchant, &None, &101, // Exceeds max limit
        &None, &None, &None,
    );
}
